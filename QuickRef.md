# Quickref
---
## Words

### Spatial Entities
#### Direction
- **tobo**: down
- **rupa**: up
- **bare**: back
- **fore**: front
- **cidari**: left
- **cikosi**: right

#### Space/Time
- **cito**: position
- **pase**: space
- **pasecito**: position in space
- **cika**: time.
- **cikacito**: position in time.

### Temporal Entities
#### Time Units
- **roira**: hour.
- **roimita**: half of hour.
- **moira**: minute.
- **moimita**: half of minute.
- **soira**: second.
- **soimita**: half of second.
- **daite**: day.
- **daimita**: half-day.
- **noite**: night.
- **noimita**: half-night.
- **guise**: week.
- **guitesa**: weekday.
- **guigedo**: weekend.
- **meiso**: month.
- **naiso**: year.
- **naigedo**: end-of-year.
- **daibare**: yesterday.
- **daifore**: tomorrow.

### Quantitative Entities
Words that indicate mathematical quantity or fraction.
- **mita**: half.
- **muito**: large amount.
- **nani**: any, any amount.
- **nona**: nothing.

### Physical Entities
Objects or physical substances that can be perceived or manipulated.
#### Substances
- **guare**: water/liquid.
- **likae**: liquid.
- **sosoe**: solid.
- **gasoe**: gas.

#### Objects
- **boki**: container.
- **mateguare**: umbrella.
- **kome**: food.

### Biological Entities
Everything that is a living being or part of a living being.
#### Organisms
- **nimale**: animal.
- **bugi**: insect.
- **bugitaraci**: cockroach.
- **kacoro**: dog.
- **kiti**: cat.
- **luperi**: wolf.
- **peteo**: pet.

#### Life/Death States
- **biba**: life.
- **cisita**: death.

#### Social Relations
- **mama:** parents
- **nisa**: sibling
- **nisalesu**: only child

### Mental / Cognitive Entities
Concepts linked to mind, thought, knowledge, the cognitive.
#### Mind and Thought
- **boru**: mind
- **borulesu**: brainless
- **gaca**: understanding

#### Knowledge
- **seno**: knowledge

### Linguistic Entities
- **toki**: conversation
- **kotoba**: word
- **deci**: saying

### Abstract Process Entities
#### Space
- **labe:** exit 
- **tine:** entry
- **sate**: being
- **sesu**: state
- **kore**: running
- **ruku**: walking
- **geta**: getting
- **kone**: returning
- **turi**: changing
- **mate**: waiting/stopping
- **dabure**: silence. Silence as a state or process.
- **goke**: joke. Joke as an event.
- **loki**: look. Look as the act of looking.
- **kareto**: creation. Creation as a process.
- **tese**: start. Start as a point in a process.
- **gedo**: end. End as a point in a process.

### Abstract Entities
- **kosa**: unknown thing
- **reale**: reality
- **teno**: possession
- **raeso**: reason
- **rala**: relation

### Possibility Entities
- **guane**: something wanted
- **nese**: something needed
- **pomo**: permission
### Social / Functional Entities
- **todoci**: friend/ally
- **guoka**: work

### Grammatical entities
#### Auxiliary verbs
- **ni keca**: invert the
#### Number specifier
- **sina**: plural

---
### Numbers
- **nona**: 0.
- **noe**: 1.
- **doe**: 2.
- **toe**: 3.
- **foe**: 4.
- **koe**: 5.
- **sae**: 6.
- **tae**: 7.
- **cae**: 8.
- **roe**: 9.
- **sana**: 10.
- **kana**: 100.
- **rana**: 1000.
- **gana**: 10000.
- **dana**: 100000.
- **tana**: 1000000.

---

## Particles
### booleans
- si: affirmation, yes

- no: negation, no, not
### Pronouns
- mi(s): first person pronoun

- tu(s): second person pronoun

- le(s): third person pronoun 

### Noun-to-verb mood 
- ni: noun-to-verb particle, mood specifier. Mood: informing

- ka:  noun-to-verb particle, mood specifier. Mood: questioning 

- go: noun-to-verb particle, mood specifier. Mood: commanding 

### Tense 
- pa: tense specifier particle: past

- fu: tense specifier particle: future  
### Aspect 
- te:  iniziative 

- ge: terminative

- na: imperfect, continuous 

### Sentences modifier
- li: object specifier. Noun in the right is the object of the verb in the current sentence.

- ke: indirect object specifier. Noun in the right is the destination of the sentence.

- ko: relative clause closing marker 

## Preposition
- du: possessive specifier. Noun in the left is possessed by noun in the right (ex.: kani du mi) 

### Conjunctions
- so: causal forward link | cause -> effect: 

- ma: contrastive adversative, opposition between both sentences:

- ta: sequential temporal link:

- bo: causal backward link |  effect -> cause: 

- fi: if-statement. Use it with "so" to create subordinator composed sentences
### Prepositions
- ro: OR relationship between nouns

- ci: AND relationship between nouns

- co: XOR relationship between nouns

- be: A be B -> A inside B

- du: A du B -> A is part of B 

- gu: A gu B -> A is with B 

- bi: A bi B -> A is made by B

- to: A to B -> A for/because B. The reason of A is B 

- ne: A ne B -> A when/at B. A at the time of B